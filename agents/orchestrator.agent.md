...
→ Delegate `security-check` and `technical-writer` simultaneously. Aggregate both results.
...
→ Delegate `troubleshooting` first. When fix is known, delegate `code-simplifier` and `changelog` simultaneously (both depend on the fix outcome, but not on each other).
